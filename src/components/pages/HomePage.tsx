...
export default function HomePage() {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Featured */}
      <Section title="Featured Projects" icon={<SparklesIcon className="w-3.5 h-3.5 text-zinc-400" />}>
        {isLoading ? (
          <LoadingSkeleton />
        ) : (
      ...
      {/* Popular */}
      <Section title="Popular Repositories" icon={<TrendingUpIcon className="w-3.5 h-3.5 text-zinc-400" />}>
        {isLoading ? (
          <LoadingSkeleton />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
      ...
      {/* AI Tools */}
      <Section title="AI & Machine Learning" icon={<CpuIcon className="w-3.5 h-3.5 text-zinc-400" />} showAll="ai-tools">
        {isLoading ? (
          <LoadingSkeleton />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
    ...
    <section>
      <div className="flex items-center justify-between mb-3.5">
        <div className="flex items-center gap-2">
          {icon}
          <h2 className="text-xs font-semibold text-zinc-800 dark:text-zinc-200 uppercase tracking-wider">{title}</h2>
        </div>
        {showAll && (
          <button
            onClick={() => navigate('category', { categoryId: showAll })}
            className="flex items-center gap-1 text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors cursor-pointer"
          >
            <span>Explore</span>
