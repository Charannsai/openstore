    ...
    <div className="space-y-10">
      ...
      >
        <div className="relative text-center max-w-2xl mx-auto mb-8">
          ...
          <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 mb-6 font-medium">
      ...
      {/* Featured */}
      <Section title="Featured Projects" icon={<SparklesIcon className="w-4 h-4 text-zinc-700 dark:text-zinc-300" />}>
        {isLoading ? (
          <LoadingSkeleton />
        ) : (
      ...
      {/* Popular */}
      <Section title="Popular Repositories" icon={<TrendingUpIcon className="w-4 h-4 text-zinc-700 dark:text-zinc-300" />}>
        {isLoading ? (
          <LoadingSkeleton />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
      ...
      {/* AI Tools */}
      <Section title="AI & Machine Learning" icon={<CpuIcon className="w-4 h-4 text-zinc-700 dark:text-zinc-300" />} showAll="ai-tools">
        {isLoading ? (
          <LoadingSkeleton />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
    ...
    <section>
      <div className="flex items-center justify-between mb-3.5">
        <div className="flex items-center gap-2">
          {icon}
          <h2 className="text-xs font-extrabold text-zinc-950 dark:text-zinc-200 uppercase tracking-wider">{title}</h2>
        </div>
        {showAll && (
          <button
            onClick={() => navigate('category', { categoryId: showAll })}
            className="flex items-center gap-1 text-xs font-semibold text-zinc-600 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-white transition-colors cursor-pointer"
          >
            <span>Explore</span>
