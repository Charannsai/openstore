  ...
  return (
    <aside className="fixed left-4 top-4 bottom-4 w-[240px] z-40 select-none">
      ...
      <div className="w-full h-full crazy-sidebar rounded-[24px] flex flex-col justify-between p-3.5 relative overflow-hidden transition-all duration-200">
        ...
        <div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className="text-sm font-bold tracking-tight text-zinc-950 dark:text-white block">
                {BRAND.name}
              </span>
                ...
                >
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute inset-0 rounded-xl bg-zinc-100 dark:bg-white/[0.08] border border-zinc-200 dark:border-white/10 shadow-sm"
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute left-1 top-1/2 -translate-y-1/2 w-1 h-4 bg-zinc-950 dark:bg-white rounded-full shadow-sm"
                      transition={{ type: 'spring', stiffness: 500, damping: 32 }}
                    />
                  )}
                  ...
                  {item.id === 'updates' && updatesCount > 0 && (
                    <span className="ml-auto z-10 bg-zinc-950 text-white dark:bg-white dark:text-zinc-950 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                      {updatesCount}
                    </span>
                  )}
                  {item.id === 'my-apps' && installedCount > 0 && (
                    <span className="ml-auto z-10 text-[10px] font-bold px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-white/10">
                      {installedCount}
                    </span>
                  )}
                </button>
        ...
        <div className="space-y-2.5 pt-3 border-t border-zinc-200 dark:border-white/[0.06]">
          {/* Theme Toggle Button */}
          <button
            onClick={toggleTheme}
          ...
          >
            ...
            <div className="w-7 h-4 rounded-full bg-zinc-300 dark:bg-zinc-700 p-0.5 relative transition-colors">
              <motion.div
                ...
                className="w-3 h-3 rounded-full bg-zinc-950 dark:bg-white shadow-sm"
              />
            </div>
          </button>

          {/* System Status Card */}
          <div className="p-3 rounded-xl bg-zinc-100/90 dark:bg-zinc-900/50 border border-zinc-200 dark:border-white/10">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-600 dark:bg-emerald-400 pulse-dot" />
                <span className="text-[11px] font-bold text-zinc-950 dark:text-zinc-100">
                  Windows x64
                </span>
              </div>
            ...
            <p className="text-[10px] text-zinc-600 dark:text-zinc-400 font-medium">Local Agent Active</p>
          </div>
        </div>
